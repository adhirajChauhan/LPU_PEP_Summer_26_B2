...
class Solution {
  ...
  bool dfsHelper(int start, vector<vector<int>> &adj, vector<bool> &visited, unordered_map<int,int> &parent){
      visited[start] = true;
      
      for(int neighbour : adj[start]){
          if(!visited[neighbour]){
              parent[neighbour] = start;
              if(dfsHelper(neighbour, adj, visited, parent)) return true;
          }
          else if(parent[start] != neighbour) return true;
      }
      return false;
  }
    ...
    bool isCycle(int V, vector<vector<int>>& edges) {
        ...
        for(int i = 0; i < V; i++){
            if(!visited[i]){
                unordered_map<int, int> parent;
                parent[i] = -1;
                if(dfsHelper(i, adj, visited, parent)){
                    return true;
                }
            }
        }
...
class Solution {
  public:
    vector<int> topoSort(int V, vector<vector<int>>& edges) {
        ...
        //if any neighbour becomes indegreee == 0, push that in queue
        
        //number of visited nodes != number of vertices -> cycle detected
        vector<vector<int> adj(V);
        for(auto i : edges){
            int u = i[0];
            int v = i[1];
